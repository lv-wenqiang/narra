...
fn strict_decodes_real_mp3_to_some() {
    let d = panda::duration::mp3_duration_seconds_strict(Path::new("tests/fixtures/tone_3s5.mp3"));
...
fn strict_returns_none_for_garbage_bytes() {
    ...
    let d = panda::duration::mp3_duration_seconds_strict(Path::new("/tmp/not_audio_strict.mp3"));
