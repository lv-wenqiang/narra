...
给一份文稿，它产出一支 1280×720 / 30fps 的 mp4：Edge TTS 合成旁白与字幕，
...
验收，242 个测试通过（另有 5 个 `#[ignore]` 的重量级用例，需要 ffmpeg 与本地
素材，手动跑）。
...
- **视频**：背景视频循环播放 → 缩放裁切到 1280×720 → 压暗到 80% → `overlay` 覆盖层 → H.264
- **音频**：TTS 旁白、背景音乐（0.15 音量，末尾 2 秒线性淡出）、打字机音效、片尾音效，四路 `amix`
- 四路进 `amix` 前统一到 48kHz 立体声；单声道的 TTS 用 `pan` 上混，不引入衰减
