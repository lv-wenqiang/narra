...
- 主探针文本「文字渲染器 Test 123」：14 个字形，其中 12 个取到非空轮廓（2 个是空格，
  正常没有轮廓）；近纯白像素 10806 个、近纯黑像素 8326 个（描边和填充都生效）；
  非透明前景包围盒 663×72px（9 个可见字符，70px 字号，在预期的 500~800px 量级内）。
- 肉眼核对 `text_probe.png`：白字黑边的「文字渲染器 Test 123」正确显示，中文笔画
  清晰可辨，不是豆腐块方框；描边均匀包裹字形，没有断裂或自相交伪影。
- **豆腐块专项核对**：另外分别渲染「文字」（`text_probe_cjk_a.png`）和「渲染」
  （`text_probe_cjk_b.png`），逐字节比较两张同尺寸图片的像素数据——22700 字节不同
  ...
  相同，证明确实取到了「文」「字」「渲」「染」各自不同的字形轮廓，不是同一个豆腐块
  被复制了四次。
...
> **2026-09-05 更正探针判据**：本节两组实验的判据此前拿字体的**家族名**
> （`LXGW WenKai Lite`）去和 cosmic-text 报回的 **PostScript 名**
> （`LXGWWenKaiLite-Regular`）比较——两者恒不相等，第 1 组的「复现成功」因此
> **恒真**、第 2 组的 PASS **永远判不出来**（当时输出的是 FAIL，尽管证据行里
> `glyph_id=0` + 内嵌字体本身就说明规避方案是有效的）。判据已改为取 name ID 6
> 的 PostScript 名，两组结论都不变，只是现在真的被判据支持了。
...
**验证**（`examples/text_probe.rs`，`main_text` 换成较短的"文字渲染器"、
...
合成粗体核对：对比同一段文字 bold=true / bold=false 的墨迹量……
  bold=false 墨迹像素数：10245
  bold=true  墨迹像素数：12611
  比例：1.231
  [PASS] 加粗版墨迹明显更多（1.231 倍），且未超过 2.5 倍上限
```

加粗版比非加粗版墨迹多约 23.1%，肉眼比对 `text_probe.png`（主探针文本用的就是
...
产出 `text_probe.png`（主探针图）、`text_probe_cjk_a.png`（"文字"）、
`text_probe_cjk_b.png`（"渲染"），均已加入 `.gitignore`（渲染产物不入库，只有生成
它们的代码入库）。
