...
> **命名说明**：本文档记录的命令行与临时路径写于工具名还是 `panda` 的时期
> （2026-09-05 改名为 `narra`）。这些是实跑留下的记录，**原样保留**——把它们
> 改成新名字，等于声称「这条命令我用 narra 跑过」，而事实并非如此。命令的语义
> 没有变化，把 `panda` 读作 `narra` 即可。
...
## 12. 把渲染与写出拆成两个线程：实测收益与没拿到的那一半（2026-09-05）

§11.4 据分摊结果提了第一件该做的事——让渲染与 ffmpeg 真正并行。这一节记它的
实测结果，**包括预测错了的那部分**。

实现见 `src/render/stream.rs`：渲染留在调用线程，写出挪到作用域线程，中间一个
容量 `CAPACITY = 2` 的有界通道，缓冲区在两端之间循环复用（不是每帧新分配——
一帧 8.3MB，新分配意味着每帧多约 2048 个缺页中断）。内存代价
`(CAPACITY + 1) × 8.3MB ≈ 25MB`。`run_render` 里 stdin/stderr 的编排一行没动，
§6 那套「stderr 必须与写帧并发读」的结论原样成立。

### 12.1 收益（交替 A/B，同一时段轮流跑，避免拿不同时段的数字对比）

| 画幅 | 串行（改前） | 流水线（改后） | |
|---|---|---|---|
| LANDSCAPE，3 轮 | 21.137 / 21.232 / 21.639s（均值 **21.34s**） | 16.957 / 17.066 / 17.112s（均值 **17.05s**） | **−20%** |
| PORTRAIT，2 轮 | 20.749 / 23.307s（均值 22.03s） | 17.942 / 19.744s（均值 18.84s） | −14%（这批机器负载已升到 5~7，噪声明显更大） |

LANDSCAPE 折算：**36.98 → 46.27 fps**，实时倍率 **1.23× → 1.54×**。

CPU 时间对照（含 ffmpeg 子进程）：串行 `real 20.689 / user 79.943 / sys 9.580`，
流水线 `real 16.893 / user 81.880 / sys 10.518`。**总 CPU 基本没变，挂钟少了 20%**
——拿到的是重叠，不是多烧的 CPU。

### 12.2 §11.4 预测的「约 13.5~14s / 56fps」没有拿到，只到 17.05s

预测的依据是「完全重叠后挂钟 = max(渲染 13.53s, ffmpeg 7.82s)」。实测停在
17.05s，**差的那 3.5s 几乎正好等于 §11 量到的管道拷贝成本**（C − B = 3.41s）。

排除掉的解释：**不是通道太小、也不是抖动没吸收住**。容量扫描（其余不变，各 2 次）：
...
**完全平坦**。所以容量取 2 不是调出来的最优值，只是「够用且内存最省」——真要
省内存，取 1 也一样快。

**剩下那 3.5s 的机制没有查实**。手上的两条线索一致指向「写端搬那 6.5GB 与渲染
线程在内存带宽 / LLC 上互相挤」——两者都是内存密集型，挤在一起就不是
`max(a, b)` 而更接近 `a + b`——但这只是**假说**，本次没有做能证伪它的实验
（比如让渲染探针与一个纯 memcpy 负载并发跑，看它是否同比例变慢）。按本文档
§6.2 与 §10.3 的教训：**先记成假说，别当结论**。

### 12.3 现在的瓶颈排序变了

改完之后，17.05s 里渲染侧自己占 13.53s（79%）。`docs/follow-ups.md` 帧渲染节
那条 `draw_centered`（约 3× 的乘数）**从此是直接收益**，不再被串行结构稀释——
它是下一件该做的事。

---

## 13. `draw_centered` 的两趟全画布操作（2026-09-05）

§12.3 记下「改完流水线之后，17.05s 里渲染侧自己占 79%，`draw_centered` 那条
从此是直接收益」。这一节记那条的落地。

`docs/follow-ups.md` 帧渲染节原条目提了两个互不冲突的局部修法，这里两个都做了：
**(b) 组透明度为 1 时跳过暂存画布**、**(a) 需要暂存画布时只分配文字包围盒那么大**。

### 13.1 每次调用省下的是什么

`draw_centered` 原来无条件走「清空整幅暂存画布 → 三遍描边/填充画进去 → 整幅
合成回目标」。清空与合成这**两趟全画布操作的代价与文字占多大面积无关**——一条
字幕只占画布百分之几，却要为整幅 1920×1080 各付一次。

实跑一条完整时间轴（789 帧）共调用 `draw_centered` **761 次**，其中
**634 次组透明度正好是 1**（入场动画走完之后就一直是 1）、127 次小于 1。
两条路径的成本差由前后两次实测反推：`(11.99 - 7.43)s / 634 ≈ **7.19ms/次**。

### 13.2 逐段实测（渲染侧单独，LANDSCAPE，`tests/render_throughput.rs`）

| 段（帧数） | 改前 | +全不透明快路径 | +包围盒暂存画布 |
|---|---|---|---|
| Cover（15） | 18.31 ms/帧 | 11.12 | **2.29** |
| Intro（105） | 12.10 | 4.35 | **0.98** |
| Content（549） | 16.79 | 11.09 | **10.08** |
| Outro（120） | 10.18 | 5.95 | **4.00** |
| **合计** | **11.99s / 65.8fps** | **7.43s / 106.3fps** | **6.15s / 128.2fps** |

Content 段降幅相对小，是因为它每帧只画一条字幕、且大部分帧本来就走快路径，
剩下的 10.08ms 里绝大部分是字形栅格化本身，不再是全画布开销。

### 13.3 端到端

| | 挂钟 | fps | 实时倍率 |
|---|---|---|---|
| 会话开始（串行 + 全画布暂存） | 21.34s | 36.98 | 1.23× |
| + 渲染/写出流水线（§12） | 17.05s | 46.27 | 1.54× |
| + 全不透明快路径 | 12.53s | 62.97 | 2.10× |
| + 包围盒暂存画布 | **11.98s** | **65.86** | **2.20×** |

累计 **−44% 挂钟**，实时余量从「1.23× 勉强够用」变成 **2.20×**。

### 13.4 正确性是怎么守住的

两个修法都要求输出**逐字节不变**，靠三层：

1. `tests/canvas_baseline.rs` 的四帧逐字节门禁（既有）。
2. 新增三条等价性测试（`src/render/text.rs`）：全不透明路径与「画在透明层上再
   整层合成」逐字节相同（白底与透明底各验一次）；半透明路径与整幅暂存画布的
   参照实现相差不超过 1 LSB；文字压在画布四边时同样成立（包围盒越界要夹到画布
   内，且只能整数平移——非整数平移会改变抗锯齿覆盖率）。
3. 变异验证：`margin` 去掉描边外扩、包围盒改四舍五入取整、平移量加 0.3 像素，
   三次各自见红。

**其中一条值得单独记**：前两个变异（`margin=0`、四舍五入）**逐字节门禁没抓住**，
只有新加的等价性测试抓住了。原因是门禁那四帧里唯一走慢路径的是 Outro 那帧，
而它那段文字没有描边，`margin` 取 0 对它没有影响。**「有一道逐字节门禁」不等于
「这次改动被门禁覆盖了」**——覆盖到的是哪条路径，得具体看。

---

## 14. 单声道素材被功率保持上混轻了 3 dB（2026-09-05 真实出片验证发现）

§9.7（裁定 R-F9）修的是 TTS 那一路的 `mono → stereo` 上混衰减，修法是把那一路
从 `channel_layouts=stereo` 换成 `pan=stereo|c0=c0|c1=c0`。**同样的缺陷此后在
片尾音效那一路重新出现**，这次是素材换了：`assets/intro.mp3` 从 `5b8f9f2`
起是**单声道**（该提交的说明写的是「只涉及来源、不夹带行为变化」——但声道数
变了，这本身就是行为变化），而它走的是三路共用的 `MIX_FORMAT`。

### 14.1 发现经过与证据

跑一次完整的 `just make` 之后逐窗测电平（`volumedetect`）：

| 支路 | 源 | 成片 | 实测差 | 规格 |
|---|---|---|---|---|
| 打字机（源立体声） | −36.3 dB | −40.8 dB | −4.50 dB | −4.44 ✓ |
| **片尾「叮」（源单声道）** | −30.9 dB | −38.3 dB | **−7.40 dB** | −4.44 ✗ |

隔离验证（同一素材，只过一个滤镜，与自身原始电平比）：

| 滤镜 | 单声道素材 | 立体声素材 |
|---|---|---|
| `aformat=sample_rates=48000:channel_layouts=stereo` | **−3.00 dB** | 0 dB |
| `pan=stereo\|c0=c0\|c1=c0` | **+0.00 dB** | 右声道丢失（塌成左声道） |
| `aresample=48000:ochl=stereo:rematrix_volume=1` | **−3.00 dB** | 左右保持 |

第三行是**试过并否掉**的第三条路：`rematrix_volume` 不影响这条上混路径，
所以**不存在层无关的单一写法**——两种写法各自只对一种输入正确。

### 14.2 为什么一条测试都没变红

所有相关断言看的都是**滤镜图字符串**，而那串对立体声输入完全正确。声道数是
**素材的属性**，不在字符串里；换素材的提交也没有任何测试去看它。

### 14.3 修法与新判据

新增 `AudioLayouts`：`probe()` 用 `ffprobe` 逐路探声道数（问 ffmpeg 自己的工具，
而不是仓库里那套 symphonia——要回答的是「ffmpeg 会把它看成几声道」），
`mix_format_for()` 按声道数逐路选写法。探不出来一律按**立体声**兜底：两种猜错
的代价不对称，把单声道当立体声只是轻 3 dB，把立体声当单声道会**丢掉右声道**。

**顺带修掉一个尚未发作的同类缺陷**：TTS 那一路此前**无条件**用 `pan`（Edge TTS
恒为单声道，对自产音频成立），但 `panda render --audio` 收的是用户任意文件——
传一份立体声进来右声道会整个消失。现在这一路也按探测结果走。

复测（真实出片）：片尾音效 **−7.40 → −4.40 dB**（规格 −4.44，偏差 **+0.04 dB**），
打字机保持 −4.50 dB，TTS 窗无 3 dB 跌落。

四条新测试：两路各按自己的声道数选写法（整条支路作连续子串断言）、立体声不得被
塌成左声道、探测读出自带两段音效的真实声道数（一单一双，正好覆盖两支）、探不出来
时兜底为立体声。
