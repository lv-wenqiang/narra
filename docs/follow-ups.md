...
当时挡在它前面的那条（渲染与 ffmpeg 不重叠）**已于同日销账**，端到端从 21.34s
降到 17.05s。改完之后 17.05s 里渲染侧自己占 13.53s（**79%**），所以这条的收益
**从此是直接的**，不再被串行结构稀释——它现在是这个方向上下一件该做的事。

（动手前仍要记得：它改的是所有像素测试的地基。两个局部修法见上面的表。）
