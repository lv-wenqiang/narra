...
渲染器**刻意禁用了系统字体回退**（见 `docs/text-rendering.md`）：字体里没有的字形
不会退到别的字体，而是画成豆腐块。而工具**不检测这件事**——README 里就写着「工具
不对此做检测」。

两条路径都会踩到：

- 用户用 `--font` 指定了一份不覆盖中文的字体（README 已明确警告，但只是警告）；
- 将来若把内嵌字体子集化（见 `docs/assets-and-licensing.md` 的体积账），生僻字、
  人名地名会开始漏。

**为什么值得做**：它的失败形式是这条链上最难受的一种——**不报错、能播放、时长
正确**，要人眼看到成片才发现，而这是一条以「无人值守出片」为目标的管线。

**修法**：排版前拿字体的 `cmap` 查一遍待渲染文本（标题、字幕、水印、品牌名），
把缺失的字符**列出来警告**（不中断出片，与 `--font` 不可用时回退内嵌字体的处置
一致）。判据测试可以拿一份只含 ASCII 的字体 + 一段中文，断言警告里列出了那些字。

**这条也是「字体要不要换小」那个问题的前置条件**：有了检测，子集化才从「坏交易」
变成「可评估的取舍」。
