...
**开工条件已于 2026-09-05 验证满足，但排在下面那条之后**（`docs/ffmpeg-pipeline.md`
§11）。原文写的是「等 ffmpeg 链路真的测出渲染是瓶颈时再做」——那次测量做了，
结论是渲染侧（LANDSCAPE 单独 58.31fps）确实比 ffmpeg 侧（100.9fps）慢，
「会被 libx264 编码整个遮住」这个当初的判断**是错的**：整个 ffmpeg 侧只占 7.82s，
其中编码不到 2.52s。

但同一次测量也发现，端到端 20.66s 里有约 3.7s（18%）纯粹丢在「渲染与 ffmpeg
没有重叠」上（见本文件 ffmpeg 合成节「值得做」第 1 条）。**那条先做**：它把
端到端从 20.66s 压到约 13.5s，之后渲染侧就是新的下限，这条的收益才不被串行结构
稀释。现在就动这条，改的是所有像素测试的地基，换来的却是被稀释过的收益。
...
#### 1. 渲染与 ffmpeg 几乎没有重叠，端到端白丢约 18%

`run_render` 的写端是单线程，`render() → write(8.3MB) → render()` 严格轮流，
而管道缓冲区只有 64KB（一帧的 1/127）——写端因此没法在 ffmpeg 消化上一帧时
提前渲染下一帧，两段被迫互等。

实测分摊（`docs/ffmpeg-pipeline.md` §11，LANDSCAPE / 789 帧）：
...
20.66 落在两端之间、偏串行那头：只重叠了约 4.1s，**白丢约 3.7s，占端到端 18%**。
这也是目前唯一一块「不用改任何渲染或编码参数就能拿回来」的时间。

**修法**：写端拆成「渲染线程 + 写线程」，中间一个容量两三帧的有界通道。上限是
端到端 20.66s → 约 13.5~14s（**约 56fps / 1.87× 实时**，约 1.5×），代价是多一个
线程与几十 MB 帧缓冲。

**动手前必须重读 `docs/ffmpeg-pipeline.md` §6 的死锁分析**：写端从一个线程变成
两个之后，`drop(stdin)` 的时机、`wait()` 的顺序、写线程的错误怎么传回主线程，
这三件事都要重新过一遍——§6 当初只要求「stderr 与写帧并发所以不死锁」，没有
要求「写端与 ffmpeg 不互等」，现在要在不破坏前者的前提下拿到后者。

（顺带：管道本身的拷贝成本实测 3.41s / 占 16%，也在这条的重叠范围内。）

#### 2. `-crf` / `-preset` 这两条提吞吐的路已实测证伪，别再试
