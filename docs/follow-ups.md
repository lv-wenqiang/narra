...
#### 1. `logo.png` 与两段音效的来源与授权没有记录

`assets/logo.png`、`assets/intro.mp3`、`assets/intro_typewriter.mp3` 三项都是
`include_bytes!` 编进二进制、随每一份构建产物分发的，但**来源与授权都没有任何
记录**——和字体处在完全相同的位置。

2026-09-05 处理字体时确认了这一点：旧字体 `dingliesongtypeface.ttf` 的 name 表
里只有「Copyright reserved by MiaoKeming」、无授权声明，已替换为霞鹜文楷 Lite
（SIL OFL 1.1）。**这三项没有一并处理**，因为字体的授权状态能从文件元数据里读
出来，而 png 和 mp3 读不出等价信息，只有当初下载它们的人知道来源。

**为什么值得做**：如果要公开发布成片、或分发构建产物，这三项是仅剩的授权盲区。
风险等级低于当初的字体（体积小、可辨识度低），但性质相同。

**怎么做**：回忆或查出三者来源，填进 `docs/assets-and-licensing.md` §1 的表格；
来源确实查不到的，用该文档 §3/§4 列出的免费素材站替换掉，替换成本很低。

**注意**：替换 logo 会打破逐字节回归门禁（`tests/canvas_baseline.rs`），按该文件
里写明的流程重新生成基线；替换音效则会动到 `assets.rs` 里那条「片尾音效应大于打字
机音效」的长度断言（它是用来抓 `include_bytes!` 路径写反的），需要重新实测。
