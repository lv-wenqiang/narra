...
- **两段音效**：改为 `tools/gen_sfx.py` 自制合成，**不涉及第三方素材**，脚本用固定
  种子可确定性复现
...
**未决**：合成音的**听感未经人工确认**——无法试听，也没有参考音源比对。

**一处已更正的判断**：当初把「音效站不可达」写成了替换为合成音的理由，当天复核
推翻——`mixkit.co` 与 `freesound.org` 均可达，Mixkit 的 mp3 实测能下载，只有
Pixabay 是 403。错因是拿猜测的 CDN 直链探测、失败后外推为整站不可达。更正详见
`docs/assets-and-licensing.md`。

**但结论未变，理由换了个更硬的**：内嵌素材随二进制分发，需要授权**明确允许再分发**
（CC0 / OFL / MIT / ISC），而多数免费音效站的条款恰恰禁止把素材作为独立文件再分发。
自制合成绕开了这个问题。若要换第三方音效，优先 Freesound 上的 CC0 条目（需注册申请
API key）。
